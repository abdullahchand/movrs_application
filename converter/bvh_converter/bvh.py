...
class BvhReader(object):
    ...
    def read(self):
        ...
        self._file_handle = self.filename
        # with open(self.filename, 'r') as self._file_handle:
        self.read_hierarchy()
        self.on_hierarchy(self.root)
        # self.read_motion()

    def read_motion(self, line, frame_counter = 0, dt = 0):
        # print("reading Motion Data")
        ...
        self.on_motion(frame_counter, dt)
        ...
        if len(a) > self.num_channels:
            splitter_start = 0
            splitter_end = self.num_channels
            while splitter_end < len(a):
                temp = a[splitter_start:splitter_end+1]
