	...
	parse: function ( buffer ) {
		...
		function readBvh( lines ) {
			...
			if(header_loaded ==0 ){
				...
				if ( nextLine( lines ) !== "HIERARCHY" ) {
				 ...
				 list = []; // collects flat array of all bones
				 root = readNode( lines, nextLine( lines ), list );
		...
		*/
		function nextLine( lines ) {
			...
			var line;
			// skip empty lines
			// console.log("aaaaaaaaaaaaaa : " +lines);
			while ( ( line = lines.split(" ").shift().trim() ).length === 0 ) { }
			return line;

		}

		var scope = this;
		// console.log("abcdwwww ", buffer)
