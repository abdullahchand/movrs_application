...
class ChartWindow:
    def __init__(self, parent):
        ...
        self.ln2, = plt.plot(self.x, self.y, '-')
    ...
    def update_graph (self,frame):
        ...
        if(len(self.graph_array)>0):
            ...
            self.y.append(result[self.graph_array[0]])
            self.ln.set_data(self.x, self.y) 
            result_array.append(self.ln)

            if(len(self.graph_array)>1):
                self.y2.append(result[self.graph_array[2]])
                self.ln2.set_data(self.x, self.y2) 
                result_array.append(self.ln2)

            if(len(self.graph_array)>2):
                self.y3.append(result[self.graph_array[3]])
                self.ln3.set_data(self.x, self.y3) 
                result_array.append(self.ln3)
                # # still a lot work to do here after discusion
                # if(time_now>50):
                #     plt.axis([0, 1000, 0, 20])
                #     plt.subplots()
                # #set data to  graph 
            print(len(self.graph_array))
